...
def _validate_gradient_boosting_loss(
        loss: str,
        quantile_alpha: Optional[float],
) -> None:
    ...
    if loss in {"huber", "quantile"}:
        if quantile_alpha is None:
            raise ValueError(
                "quantile_alpha must be provided when loss is 'huber' or 'quantile'."
            )
...
def build_gradient_boosting_pipeline(
    ...
    _validate_gradient_boosting_loss(
        loss=loss,
        quantile_alpha=quantile_alpha,
    )

    gradient_boosting_params = gradient_boosting_params or {}
    ...
    if loss in {"quantile", "huber"}:
        model_params["alpha"] = 0.95 if quantile_alpha is None else float(quantile_alpha)

    model_params.update(gradient_boosting_params)
...
def get_feature_importances(
    ...
    gradient_boosting_model = pipeline.named_steps["model"]

    if not hasattr(gradient_boosting_model, "feature_importances_"):
        raise AttributeError(
            "The fitted gradient boosting model does not expose feature_importances_."
        )
